...
async function main() {
  ...
  const baseResult = {
    ...
    scope: 'chrome_extension',
    reportSource: 'not-read',
    initialReadAvailable: false,
    initialReadReportCount: 0,
    initialReadLatestTraceId: undefined,
    nextReadCommand: undefined,
  };
  ...
  const initialReadReports = Array.isArray(initialRead?.reports)
    ? initialRead.reports
    : [];
  ...
  const result = {
    ...
    initialReadReportCount: initialReadReports.length,
    initialReadLatestTraceId: initialRead?.latestReport?.traceId,
    initialRead,
    ...
    readCommand,
    nextReadCommand: readCommand,
    message:
      'Opened page with the observability init script registered before navigation. Analyze initialRead first; run nextReadCommand only after user interaction, reload, or a follow-up read target.',
  };
