...
export default function Layout() {
  ...
  const [hydrationState, setHydrationState] = useState("server stream markup");
  ...
  return (
    <main className="shell">
      <header className="topbar">
        <div>
          <p className="eyebrow">OpenRuntime Modern.js Stream SSR Demo</p>
          <h1>Stream SSR and Hydration Status</h1>
        </div>
        <nav className="nav">
          {navItems.map((item) => (
            <Link
