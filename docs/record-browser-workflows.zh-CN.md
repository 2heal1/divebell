...
`record-divebell-workflow` 是一个可以安装到 Agent 的中文 skill。它让用户先在可见浏览器里完成一次真实操作，再把操作元素、事件顺序和页面结果生成可重复运行的 JavaScript 脚本。语音说明是可选补充，不是生成脚本的前提。
...
Agent 会先运行 `divebell record start` 准备录制文件和跨页面操作采集，再通过 `divebell open about:blank --ui` 打开可见空白页面。只有用户希望补充口头意图时才增加 `--mic`。CLI 自动准备并注入 Bridge，录制扩展在同一次页面打开中注入录制脚本。录制结果默认保存到当前项目的 `recordings/` 目录，用户不需要提前提供网址或保存位置。

接下来正常操作浏览器即可：

1. 打开目标网页并完成点击、输入和页面跳转。
2. 如果操作本身不能表达最终目标，可以在聊天中补充一句；也可以选择通过麦克风说明。
...
生成的脚本会逐步等待录制时的操作元素，完成输入、下拉选择、按键和点击，并检查最终页面是否到达录制结束时的状态。密码和文件路径不会写进录制包，运行到这类步骤时会明确要求调用方重新提供。
