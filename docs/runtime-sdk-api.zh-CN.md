...
English version: [Runtime SDK API](runtime-sdk-api.md)

`@divebell/core` 是 Divebell 的页面侧 API。它让应用主动提供浏览器表面无法稳定获得的内部状态、关键事件、声明动作和等待条件。

Runtime SDK 是可选增强，不是使用 Divebell CLI、登录状态、浏览器调试或 Extensions 的前置条件。

## 什么时候使用

适合接入 Runtime SDK 的情况：
...
Target ID 应稳定、唯一、可读。`type` 和 `statuses` 由接入方声明，Runtime SDK 不内置固定业务类型或状态。
...
页面没有 Runtime SDK 时，这些命令不会提供应用内部信息，但浏览器调试和 Extension 仍然可以正常使用。

## Runtime SDK 与 Extension 的区别
...
| 页面内部业务状态和阻塞关系 | Runtime SDK |
| 页面允许 Agent 执行的稳定业务动作 | Runtime SDK |

Extension 在页面外部组织开发调试流程；Runtime SDK 在页面内部提供事实。两者可以组合，但不要求同时使用。
