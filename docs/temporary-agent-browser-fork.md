...
This package comes from a temporary agent-browser branch and adds the memory diagnostics and code-coverage capture required by Divebell. Divebell uses the executable included with the CLI unless `DIVEBELL_AGENT_BROWSER_EXECUTABLE` points to a custom build.
