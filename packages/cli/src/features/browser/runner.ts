...
const AGENT_BROWSER_RESTORE_INITIAL_SAVE_DEFAULT_ENV = "AGENT_BROWSER_RESTORE_INITIAL_SAVE_DEFAULT";
const AGENT_BROWSER_RESTORE_PERIODIC_SAVE_DEFAULT_ENV = "AGENT_BROWSER_RESTORE_PERIODIC_SAVE_DEFAULT";
const AGENT_BROWSER_RESTORE_CLOSE_SAVE_DEFAULT_ENV = "AGENT_BROWSER_RESTORE_CLOSE_SAVE_DEFAULT";
const AGENT_BROWSER_RESTORE_PERIODIC_SAVE_INTERVAL_DEFAULT_ENV = "AGENT_BROWSER_RESTORE_PERIODIC_SAVE_INTERVAL_MS_DEFAULT";
...
export function createAgentBrowserEnvironment(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    // These are caller defaults, intentionally below agent-browser CLI flags,
    // environment variables, and config-file values. They are parsed by every
    // client command and sent to the daemon, so an already-running daemon also
    // receives the current Divebell policy.
    [AGENT_BROWSER_RESTORE_INITIAL_SAVE_DEFAULT_ENV]: "true",
    [AGENT_BROWSER_RESTORE_PERIODIC_SAVE_DEFAULT_ENV]: "false",
    [AGENT_BROWSER_RESTORE_CLOSE_SAVE_DEFAULT_ENV]: "true",
    [AGENT_BROWSER_RESTORE_PERIODIC_SAVE_INTERVAL_DEFAULT_ENV]: "30000"
  };
  if (options.disableRestore === true) {
    delete env[AGENT_BROWSER_RESTORE_ENV];
  }
  if (options.ignoreConfiguredProfile === true) {
    delete env[AGENT_BROWSER_PROFILE_ENV];
  }
  if (options.ignoreConfiguredState === true) {
    delete env[AGENT_BROWSER_STATE_ENV];
  }
  if (options.defaultTimeoutMs !== undefined) {
    env[AGENT_BROWSER_DEFAULT_TIMEOUT_ENV] = String(options.defaultTimeoutMs);
  }
