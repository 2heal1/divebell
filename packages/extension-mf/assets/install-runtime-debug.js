...
//#endregion
//#region ../sdk/dist/generateSnapshotFromManifest.js
...
//#region ../sdk/dist/node.js
	...
	const createScriptNode = typeof ENV_TARGET === "undefined" || ENV_TARGET !== "web" ? (url, cb, attrs, loaderHook) => {
		...
		const handleScriptFetch = async (f, urlObj) => {
			try {
				const data = await (await f(urlObj.href)).text();
...
//#endregion
//#region ../error-codes/dist/browser.mjs
...
//#endregion
//#region ../runtime-core/dist/utils/logger.js
...
//#endregion
//#region ../runtime-core/dist/utils/tool.js
...
//#endregion
//#region ../runtime-core/dist/global.js
	...
	function setGlobalFederationConstructor(FederationConstructor, isDebug = isDebugMode()) {
		if (isDebug) {
			...
			CurrentGlobal.__FEDERATION__.__DEBUG_CONSTRUCTOR_VERSION__ = "2.8.0";
		}
	}
	...
	const registerGlobalPlugins = (plugins) => {
		...
		if (registeredPlugins.length > 0) new Set([...CurrentGlobal.__FEDERATION__.__INSTANCES__, ...nativeGlobal.__FEDERATION__.__INSTANCES__]).forEach((instance) => {
...
//#endregion
//#region ../runtime-core/dist/utils/semver/constants.js
...
//#endregion
//#region ../runtime-core/dist/utils/semver/utils.js
...
//#endregion
//#region ../runtime-core/dist/utils/semver/parser.js
...
//#endregion
//#region ../runtime-core/dist/utils/semver/compare.js
...
//#endregion
//#region ../runtime-core/dist/utils/semver/index.js
...
//#endregion
//#region ../runtime-core/dist/constant.js
	const DEFAULT_SCOPE = "default";
	const DEFAULT_REMOTE_TYPE = "global";

//#endregion
//#region ../runtime-core/dist/utils/share.js
	...
	*/
	...
	const isVersionCompatible = (version, requiredVersion) => {
	...
	function getSharedCandidateInfo(scope, version, shared, requiredVersion) {
		const compatible = isVersionCompatible(version, requiredVersion);
		const treeShakingShared = shared.treeShaking;
		const loaded = isLoaded(shared) || Boolean(treeShakingShared && isLoaded(treeShakingShared));
		const loading = !loaded && (isLoading(shared) || Boolean(treeShakingShared && isLoading(treeShakingShared)));
		return {
			scope,
			version,
	...
	function getSharedCandidates(shareScopeMap, scope, pkgName, requiredVersion) {
		return Object.entries(shareScopeMap[scope]?.[pkgName] || {}).map(([version, shared]) => getSharedCandidateInfo(scope, version, shared, requiredVersion));
	}
	...
	function getRegisteredShare(localShareScopeMap, pkgName, shareInfo, resolveShare, onSelection) {
		if (!localShareScopeMap) return;
		const { shareConfig, scope = DEFAULT_SCOPE, strategy, treeShaking } = shareInfo;
		const scopes = Array.isArray(scope) ? scope : [scope];
		const createDecision = (sc, candidates, reason, selected, failureReason) => {
			const selectedCandidate = selected ? {
				...selected,
				rejectionReason: void 0
			} : void 0;
			const candidatesWithReasons = candidates.map((candidate) => {
				if (Boolean(selectedCandidate && candidate.scope === selectedCandidate.scope && candidate.version === selectedCandidate.version && candidate.provider === selectedCandidate.provider)) return selectedCandidate;
				if (candidate.rejectionReason) return candidate;
				let rejectionReason;
				if (reason === "custom-resolver") rejectionReason = "custom-resolver";
				else if (reason === "singleton-existing") rejectionReason = "singleton-existing";
				else if (reason === "loaded-first" && !candidate.loaded && !candidate.loading) rejectionReason = "not-loaded";
				else if (selectedCandidate) rejectionReason = "lower-priority-version";
		...
		for (const sc of scopes) if (shareConfig && localShareScopeMap[sc] && localShareScopeMap[sc][pkgName]) {
			...
			const candidates = getSharedCandidates(localShareScopeMap, sc, pkgName, requiredVersion);
			...
			try {
				...
				if (!resolved) {
					onSelection?.(createDecision(sc, candidates, "version-mismatch", void 0, "version-mismatch"));
					return;
				}
				const selected = getSharedCandidateInfo(sc, resolved.shared.version, resolved.shared, requiredVersion);
				let reason;
				if (usedCustomResolver) reason = "custom-resolver";
				else if (shareConfig.singleton) reason = "singleton-existing";
				else if (strategy === "loaded-first" && isLoadingOrLoaded(resolved.shared)) reason = "loaded-first";
				else if (resolved.shared.version === shareInfo.version) reason = "exact-match";
				else if (requiredVersion === false || requiredVersion === "*") reason = strategy === "loaded-first" ? "loaded-first" : "version-first";
				else if (resolved.shared.version === maxOrSingletonVersion) reason = "compatible-highest-version";
				else reason = "compatible-version";
				onSelection?.(createDecision(sc, candidates, reason, selected));
				return resolved;
			} catch (selectionError) {
				const failureReason = !usedCustomResolver && Boolean(shareConfig.singleton && shareConfig.strictVersion) && typeof requiredVersion === "string" && isVersionCompatible(maxOrSingletonVersion, requiredVersion) === false ? "strict-version-rejected" : "load-error";
				onSelection?.(createDecision(sc, candidates, failureReason, void 0, failureReason));
				throw selectionError;
			}
		}
		const candidates = scopes.flatMap((sc) => getSharedCandidates(localShareScopeMap, sc, pkgName, shareConfig?.requiredVersion));
		const failureReason = shareConfig ? "missing-provider" : "missing-config";
		onSelection?.(createDecision(scopes[0], candidates, failureReason, void 0, failureReason));
	}
...
//#endregion
//#region ../runtime-core/dist/utils/manifest.js
...
//#endregion
//#region ../runtime-core/dist/utils/resource.js
	const RESOURCE_ERROR_URL_PATTERN = /https?:\/\/[^\s'"<>]+/g;
	const RESOURCE_ERROR_SENSITIVE_PATTERN = /\b(token|authorization|cookie|secret|password|session|access_token|refresh_token|api_key|apikey|key)\s*[:=]\s*([^&\s'",;<>]+)/gi;
	const RESOURCE_ERROR_ABSOLUTE_PATH_PATTERN = /(?:file:\/\/)?(?:\/(?:Users|private|var|tmp|home|workspace|opt|usr)\/[^\s)]+|[A-Za-z]:\\[^\s)]+)/g;
	function sanitizeResourceErrorText(value, maxLength = 800) {
		const sanitized = String(value).replace(RESOURCE_ERROR_URL_PATTERN, (rawUrl) => {
			try {
		...
		}).replace(RESOURCE_ERROR_SENSITIVE_PATTERN, "[redacted]").replace(RESOURCE_ERROR_ABSOLUTE_PATH_PATTERN, "[redacted-path]");
		return sanitized.length > maxLength ? `${sanitized.slice(0, maxLength)}...` : sanitized;
	}
	function createResourceLoadErrorSummary(error) {
		...
		return { message: sanitizeResourceErrorText(error) };
	}
	function getResourceLoadRemote(remoteInfo) {
	...
	function classifyResourceLoadError(error, fallback = "unknown") {
		...
		if (value.includes("scriptexecutionerror") || value.includes("execution error")) return "execution";
		if (value.includes("network") || value.includes("failed to fetch") || value.includes("cors") || value.includes("unreachable") || value.includes("enoent")) return "network";
		if (value.includes("runtime-001") || value.includes("global not found")) return "initialization";
		if (error instanceof SyntaxError || value.includes("json") || value.includes("valid federation manifest")) return "content";
		return fallback;
	}
	async function startResourceLoad(origin, options) {
		const event = {
			...options.context,
			url: options.url,
	...
	async function emitCachedResourceLoad(origin, options) {
		return (await startResourceLoad(origin, options)).finish("cached", { cacheSource: options.cacheSource });
	}
...
//#endregion
//#region ../error-codes/dist/desc.mjs
...
//#endregion
//#region ../runtime-core/dist/utils/load.js
	...
	async function getRemoteEntry(params) {
		...
		if (existingLoading) {
			...
			if (result) await emitCachedResourceLoad(origin, {
				context: effectiveResourceContext,
				url: remoteInfo.entry,
				remoteInfo,
				cacheSource: "mf-memory"
		...
		if (!globalLoading[uniqueKey]) {
			...
			const loadResource = async () => {
				...
				try {
					...
					let cached = false;
					if (!res) {
						const existingRemoteEntryExports = getRemoteEntryExports(remoteInfo.name, remoteInfo.entryGlobalName).entryExports;
						if (existingRemoteEntryExports) {
							res = existingRemoteEntryExports;
							cached = true;
						}
					}
					if (!res) res = (typeof ENV_TARGET !== "undefined" ? ENV_TARGET === "web" : origin.options.inBrowser ?? isBrowserEnvValue) ? await loadEntryDom({
						remoteInfo,
						remoteEntryExports,
						loaderHook,
						getEntryUrl,
					...
					await attempt.finish(cached ? "cached" : "success", { cacheSource: cached ? "mf-memory" : void 0 });
					return res;
				} catch (loadError) {
			...
			globalLoading[uniqueKey] = loadResource().then(async (res) => {
...
//#endregion
//#region ../runtime-core/dist/utils/env.js
	function getBuilderId$1() {
		return typeof FEDERATION_BUILD_IDENTIFIER !== "undefined" ? FEDERATION_BUILD_IDENTIFIER : "";
	}

//#endregion
//#region ../runtime-core/dist/utils/plugin.js
	const registeredPluginsByInstance = /* @__PURE__ */ new WeakMap();
	function getRegisteredPlugins(instance) {
...
//#endregion
//#region ../runtime-core/dist/utils/context.js
...
//#endregion
//#region ../runtime-core/dist/utils/preload.js
	...
	function createAssetResult(context, url, status, error, resourceResult) {
		return {
			url,
			status,
	...
	async function waitForRemoteEntryPreload(host, remoteInfo, entryRemoteInfo, context) {
		...
		if (cachedRemote?.remoteEntryExports) return createAssetResult(context, url, "cached", void 0, await emitCachedResourceLoad(host, {
			context: {
				...context,
				url
			},
	...
	function waitForLinkPreload({ host, remoteInfo, url, attrs, context, needDeleteLink }) {
		return startResourceLoad(host, {
			...
			url,
			remoteInfo
		}).then((attempt) => new Promise((resolve) => {
			let needAttach = true;
			const settle = (outcome, error) => {
				const errorType = error ? classifyResourceLoadError(error, "network") : void 0;
				attempt.finish(outcome, {
					cacheSource: outcome === "cached" ? "browser" : void 0,
					error,
					errorType
				}).then((resourceResult) => {
					resolve(createAssetResult(context, url, resourceResult.outcome, resourceResult.error, resourceResult));
				});
			};
	...
	function waitForScriptPreload({ host, remoteInfo, url, attrs, context }) {
		return startResourceLoad(host, {
			...
			needAttach && document.head.appendChild(script);
			if (!needAttach) queueMicrotask(() => settle("cached"));
		}));
	}
...
//#endregion
//#region ../runtime-core/dist/module/index.js
...
//#endregion
//#region ../runtime-core/dist/utils/hooks/pluginSystem.js
...
//#endregion
//#region ../runtime-core/dist/utils/hooks/syncHook.js
	var SyncHook = class {
		constructor(type) {
			this.type = "";
...
//#endregion
//#region ../runtime-core/dist/utils/hooks/asyncHook.js
...
//#endregion
//#region ../runtime-core/dist/utils/hooks/syncWaterfallHook.js
...
//#endregion
//#region ../runtime-core/dist/utils/hooks/asyncWaterfallHooks.js
...
//#endregion
//#region ../runtime-core/dist/remote/disabled.js
...
//#endregion
//#region ../runtime-core/dist/plugins/snapshot/index.js
...
//#endregion
//#region ../runtime-core/dist/plugins/generate-preload-assets.js
	function splitId(id) {
...
//#endregion
//#region ../runtime-core/dist/plugins/snapshot/SnapshotHandler.js
	...
	var SnapshotHandler = class {
		...
		async getManifestJson(manifestUrl, moduleInfo, extraOptions, resourceOptions) {
			const getManifest = async () => {
				...
				try {
					try {
						let res = await this.loaderHook.lifecycle.fetch.emit(manifestUrl, {}, remoteInfo, resourceContext);
						...
						if (!res.ok) httpError = /* @__PURE__ */ new Error(`Manifest request failed with HTTP status ${res.status}.`);
					...
					if (missingRequiredFields.length > 0) {
						const contentError = /* @__PURE__ */ new Error(`"${manifestUrl}" is not a valid federation manifest for remote "${moduleInfo.name}". Missing required fields: ${missingRequiredFields.join(", ")}.`);
					...
					if (httpError) await attempt.finish("error", {
						...responseInfo,
						error: httpError,
						errorType: "http"
					});
					else await attempt.finish(recovered ? "recovered" : "success", {
						...responseInfo,
						error: recovered ? originalError : void 0,
						errorType: recovered ? classifyResourceLoadError(originalError, "network") : void 0
					});
					return manifestJson;
				} catch (manifestError) {
					delete this.manifestLoading[manifestUrl];
...
//#endregion
//#region ../runtime-core/dist/plugins/snapshot/disabled.js
...
//#endregion
//#region ../runtime-core/dist/shared/index.js
	var SharedHandler = class {
		constructor(host) {
			this.hooks = new PluginSystem({
				beforeRegisterShare: new SyncWaterfallHook("beforeRegisterShare"),
				afterRegisterShare: new SyncHook("afterRegisterShare"),
		...
		createLoadContext(lifecycle, extraOptions) {
			this.operationCounter += 1;
			const context = extraOptions?.context || {};
			return {
				...context,
				operationId: context.operationId || `${lifecycle}-${this.operationCounter}`,
				trigger: context.trigger || extraOptions?.from || "runtime"
			};
		}
		createSelectionResult(decision, loadType, context, overrides = {}) {
			return {
				...decision,
				...overrides,
				loadType,
		...
		createSelectionDecision(pkgName, shareInfo, reason) {
			const scopes = shareInfo?.scope?.length ? shareInfo.scope : [DEFAULT_SCOPE];
			const candidates = scopes.flatMap((scope) => Object.entries(this.shareScopeMap[scope]?.[pkgName] || {}).map(([version, shared]) => getSharedCandidateInfo(scope, version, shared, shareInfo?.shareConfig?.requiredVersion)));
			return {
				scope: scopes[0],
				requestedVersion: shareInfo?.version,
		...
		emitAfterRegisterShare(pkgName, registration) {
		...
		emitAfterLoadShare({ lifecycle, pkgName, shareInfo, selectedShared, selectionResult }) {
			try {
		...
		emitErrorLoadShare({ lifecycle, pkgName, shareInfo, error, recovered, selectionResult }) {
			try {
			...
			Object.keys(newShareInfos).forEach((sharedKey) => {
				newShareInfos[sharedKey].forEach((sharedVal) => {
					sharedVal.scope.forEach((sc) => {
						...
						} else if (previousAtVersion === sharedVal || previousAtVersion?.from === sharedVal.from) {
							action = "reused";
							reason = "same-version-same-provider";
						} else {
							action = "ignored";
							reason = previousAtVersion ? "existing-version-preserved" : "scope-already-registered";
						}
						const candidates = Object.entries(this.shareScopeMap[sc]?.[sharedKey] || {}).map(([version, shared]) => getSharedCandidateInfo(sc, version, shared, sharedVal.shareConfig?.requiredVersion));
						const effectiveShared = this.shareScopeMap[sc]?.[sharedKey]?.[sharedVal.version] || Object.values(this.shareScopeMap[sc]?.[sharedKey] || {})[0];
						this.emitAfterRegisterShare(sharedKey, {
							scope: sc,
							trigger: "runtime",
							candidate: getSharedCandidateInfo(sc, sharedVal.version, sharedVal, sharedVal.shareConfig?.requiredVersion),
							candidates,
							action,
							effective: effectiveShared ? getSharedCandidateInfo(sc, effectiveShared.version, effectiveShared, sharedVal.shareConfig?.requiredVersion) : void 0,
							reason
						});
					});
				});
		...
		async loadShare(pkgName, extraOptions) {
			...
			try {
				...
				const { shared: registeredShared, useTreesShaking } = getRegisteredShare(this.shareScopeMap, pkgName, shareOptionsRes, this.hooks.lifecycle.resolveShare, (decision) => {
					selectionDecision = decision;
				}) || {};
				if (selectionDecision && extraOptions?.resolver && registeredShared) selectionDecision = {
					...selectionDecision,
					reason: "custom-resolver",
					failureReason: void 0
				};
				const registeredSelectionResult = registeredShared ? this.createSelectionResult(selectionDecision || this.createSelectionDecision(pkgName, resolvedShareOptions, "exact-match"), "async", loadContext, {
					selected: selectionDecision?.selected || getSharedCandidateInfo(selectionDecision?.scope || resolvedShareOptions.scope?.[0] || DEFAULT_SCOPE, registeredShared.version, registeredShared, resolvedShareOptions.shareConfig?.requiredVersion),
					failureReason: void 0
				}) : void 0;
				if (registeredShared) {
					...
					if (extraOptions?.customShareInfo) {
						this.emitErrorLoadShare({
							...
							shareInfo: resolvedShareOptions,
							recovered: true,
							selectionResult: this.createSelectionResult(selectionDecision || this.createSelectionDecision(pkgName, resolvedShareOptions, "version-mismatch"), "async", loadContext, { recovered: true })
						});
						return false;
					}
					...
					this.emitAfterLoadShare({
						...
						selectionResult: this.createSelectionResult(selectionDecision || this.createSelectionDecision(pkgName, resolvedShareOptions, "local-fallback"), "async", loadContext, {
							reason: "local-fallback",
							failureReason: void 0,
							fallback: true,
							selected: getSharedCandidateInfo(resolvedShareOptions.scope?.[0] || DEFAULT_SCOPE, resolvedShareOptions.version, resolvedShareOptions, resolvedShareOptions.shareConfig?.requiredVersion)
						})
					});
					return factory;
				}
				...
				this.emitErrorLoadShare({
					...
					error: shareError,
					selectionResult: this.createSelectionResult(selectionDecision || this.createSelectionDecision(pkgName, shareOptionsRes, shareOptionsRes ? "load-error" : "missing-config"), "async", loadContext, { failureReason: selectionDecision?.failureReason || (shareOptionsRes ? "load-error" : "missing-config") })
				});
				throw shareError;
			}
		}
		...
		*/
		initializeSharing(shareScopeName = DEFAULT_SCOPE, extraOptions) {
			...
			const register = (name, shared) => {
				...
				const activeVersion = existingShared && directShare(existingShared);
				...
				const shouldReplace = Boolean(!activeVersion || activeVersion.strategy !== "loaded-first" && !activeVersion.loaded && (Boolean(!eager) !== !activeVersionEager ? eager : hostName > versions[version].from));
				let action;
				let reason;
				if (existingShared === shared) {
					...
					reason = !activeVersion ? "first-registration" : eager && !activeVersionEager ? "eager-preferred" : "provider-name-preferred";
				} else {
					action = "ignored";
					reason = activeVersion.strategy === "loaded-first" ? "loaded-first-preserved" : activeVersion.loaded ? "loaded-version-preserved" : activeVersionEager && !eager ? "eager-provider-preserved" : "provider-name-preserved";
				}
				const candidate = getSharedCandidateInfo(shareScopeName, version, shared, shared.shareConfig?.requiredVersion);
				const effectiveShared = versions[version];
				this.emitAfterRegisterShare(name, {
					scope: shareScopeName,
					trigger,
					candidate,
					candidates: Object.entries(versions).map(([candidateVersion, candidateShared]) => getSharedCandidateInfo(shareScopeName, candidateVersion, candidateShared, shared.shareConfig?.requiredVersion)),
					action,
					effective: effectiveShared ? getSharedCandidateInfo(shareScopeName, effectiveShared.version, effectiveShared, shared.shareConfig?.requiredVersion) : void 0,
					reason
				});
			};
		...
		loadShareSync(pkgName, extraOptions) {
			...
			try {
				...
				const { shared: registeredShared } = getRegisteredShare(this.shareScopeMap, pkgName, shareOptions, this.hooks.lifecycle.resolveShare, (decision) => {
					selectionDecision = decision;
				}) || {};
				if (selectionDecision && extraOptions?.resolver && registeredShared) selectionDecision = {
					...selectionDecision,
				...
				const registeredSelectionResult = registeredShared ? this.createSelectionResult(selectionDecision || this.createSelectionDecision(pkgName, shareOptions, "exact-match"), "sync", loadContext, {
					selected: selectionDecision?.selected || getSharedCandidateInfo(selectionDecision?.scope || shareOptions.scope?.[0] || DEFAULT_SCOPE, registeredShared.version, registeredShared, shareOptions.shareConfig?.requiredVersion),
					failureReason: void 0
				}) : void 0;
				if (registeredShared) {
				...
				if (shareOptions.lib) {
					...
					this.emitAfterLoadShare({
						...
						selectionResult: this.createSelectionResult(selectionDecision || this.createSelectionDecision(pkgName, shareOptions, "local-fallback"), "sync", loadContext, {
							reason: "local-fallback",
							failureReason: void 0,
							fallback: true,
							selected: getSharedCandidateInfo(shareOptions.scope?.[0] || DEFAULT_SCOPE, shareOptions.version, shareOptions, shareOptions.shareConfig?.requiredVersion)
						})
					});
				...
				this.emitErrorLoadShare({
					...
					shareInfo: shareOptions,
					error: shareError,
					selectionResult: this.createSelectionResult(selectionDecision || this.createSelectionDecision(pkgName, shareOptions, "load-error"), "sync", loadContext, { failureReason: selectionDecision?.failureReason || "load-error" })
				});
				throw shareError;
			}
		}
		initShareScopeMap(scopeName, shareScope, extraOptions = {}) {
			...
			Object.entries(shareScope).forEach(([pkgName, versions]) => {
				Object.entries(versions).forEach(([version, shared]) => {
					...
					const action = !previousShared ? "registered" : previousShared === shared ? "reused" : "replaced";
					this.emitAfterRegisterShare(pkgName, {
						scope: scopeName,
						trigger: "container-init",
						candidate: getSharedCandidateInfo(scopeName, version, shared, shared.shareConfig?.requiredVersion),
						candidates: Object.entries(versions).map(([candidateVersion, candidateShared]) => getSharedCandidateInfo(scopeName, candidateVersion, candidateShared, shared.shareConfig?.requiredVersion)),
						action,
						effective: getSharedCandidateInfo(scopeName, version, shared, shared.shareConfig?.requiredVersion),
						reason: action === "registered" ? "container-share-registered" : action === "reused" ? "container-share-reused" : "container-share-replaced"
					});
				});
			});
...
//#endregion
//#region ../runtime-core/dist/shared/disabled.js
...
//#endregion
//#region ../runtime-core/dist/remote/index.js
...
//#endregion
//#region ../runtime-core/dist/core.js
	...
	var ModuleFederation = class {
		constructor(userOptions) {
			...
			this.loaderHook = new PluginSystem({
				beforeLoadResource: new AsyncHook("beforeLoadResource"),
				afterLoadResource: new AsyncHook("afterLoadResource"),
			...
			this.bridgeHook = new PluginSystem({
				...
				beforeBridgeOperation: new SyncHook(),
				bridgeRenderInvoked: new SyncHook(),
				afterBridgeOperation: new SyncHook(),
				afterBridgeCommit: new SyncHook()
			});
		...
		registerPlugins(plugins) {
			this.options.plugins = registerPlugins$1(plugins, this);
		}
		registerRemotes(remotes, options) {
...
//#region ../runtime-core/dist/index.js
	const Module = typeof FEDERATION_OPTIMIZE_NO_REMOTE === "boolean" && FEDERATION_OPTIMIZE_NO_REMOTE ? UnavailableRemoteModule : Module$1;
