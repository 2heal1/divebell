...
export function createCompatibilitySummary(
  ...
  if (snapshot.state.completeness.history === "partial") {
    ...
    recommendedActions.push(
      ...
      "Reopen the page before reproducing the loading path."
    );
  }
  if (snapshot.injection?.timing === "late") {
    ...
    recommendedActions.push("Reopen the page to capture the full history.");
  }
...
function moduleRecommendedActions(
  ...
  if (snapshot.state.completeness.history === "partial") {
    actions.push("Reopen the page before reproducing the remote load.");
  }
