...
Use this reference when diagnosing Module Federation remote, expose, shared, preload, or observability report state through Divebell.
...
MF consumers should use `@module-federation/observability-plugin`. The plugin records remote, expose, shared, preload, and report data that Divebell can expose as targets and actions.
...
without adding `@divebell/core`. If the project uses Module Federation or
remote/shared/expose based loading, treat missing `mf:*` Divebell state as a
...
add and wire the observability plugin before relying on Divebell for MF
...
When using `divebell verify` on `mf:*` targets, treat a ready result as
...
- a Divebell `mf.shared` / `mf.shared.conflict` target
- an MF observability report read through Divebell actions or the MF
...
```bash
divebell targets --url <url> --type mf.remote
divebell targets --url <url> --type mf.remote.expose
divebell targets --url <url> --type mf.shared
```

Wait for a concrete expose rather than only the remote summary:

```bash
divebell wait-for mf:remote:<remoteName>:expose:<exposeName> ready --url <url> --timeout 10000
```

Diagnose shared dependencies:

```bash
divebell snapshot --url <url> --query <sharedName>
divebell wait-for mf:shared:<sharedName>:<version>:<shareScope> loaded --url <url> --timeout 10000
divebell wait-for mf:shared:<sharedName>:<version>:<shareScope> error --url <url> --timeout 10000
```

## Report Actions

If MF observability registered report actions, read detailed reports through Divebell:

```bash
divebell run-action --url <url> mf:list-reports --payload '{"remote":"<remoteName>"}'
divebell run-action --url <url> mf:get-report --payload '{"traceId":"<trace-id>"}'
```
