...
- **Target**: an object the page makes referenceable or waitable, such as a
  business component, route, application dependency, or sub-application.
...
provided directly by the page or a separately installed integration.
...
- Integration target: generated and documented by the installed integration
...
A ready integration target proves only the layer represented by that
integration. It does not prove that the business UI succeeded. When an ordinary
page has no business target, verify through a directly relevant page result,
request result, or specialized Extension. Do not add a low-value target solely
to run `verify`.
