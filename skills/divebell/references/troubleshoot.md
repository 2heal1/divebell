# Troubleshoot and Fix with Divebell
...
```bash
divebell open <app-url> --session <debug-session>
```
...
command has a skill, run `divebell <command> --skill`, read it completely,
...
integration merely because the project has Divebell installed.
...
divebell network --url <relevant-query>
divebell screenshot debug-state
```
...
- Browser-visible evidence cannot reliably determine the real business state or
  blocking cause.
- The user explicitly requests Divebell integration or a long-term
  verification signal.
...
failure; adding Divebell integration is not itself the fix.
