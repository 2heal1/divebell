...
async function runConnected(args) {
  ...
  evidence.nextAction = exhausted
    ? {
        type: "blocked",
        summary: `Divebell runtime was not connected after ${MAX_ATTEMPTS} attempts.`,
...
async function runVerify(args) {
  ...
  const verifyResult = runDivebellVerify({
    targetId,
...
function runDivebellVerify(options) {
  const command = "pnpm";
  const commandArgs = [
    "exec",
    "divebell",
      ...
      parsed: {
        result: {
          ...
          evidence: {
            ...
            targetClass: "unknown",
            message: stderr.trim() || stdout.trim() || "divebell verify did not return JSON.",
            nextStep: "Install @divebell/cli, run `divebell extensions add @divebell/extension-troubleshooting`, connect the page runtime, then rerun workflow verify."
          }
        }
      }
    };
...
function createOpenPageNextAction(bridge, url, options = {}) {
  const command = url === undefined
    ? null
    : `divebell open ${quoteShellArg(url)} --bridge ${quoteShellArg(bridge)}`;
  const summary = options.bridgeReachable === false
    ? "Bridge is not reachable. Open the target page with the Divebell CLI so it can auto-start Bridge, then rerun this connected check."
    : "No divebell open operation was recorded for this working directory. Open the target page with the CLI, then rerun this connected check.";
  return {
    type: "open_page",
    summary,
    ...
    snippets: [
      {
        ...
        language: "ts",
        code: `import { createDivebell, installDivebellOnWindow } from "@divebell/core";
...
function createObserveNextAction() {
  return {
    type: "run_observe",
    summary: "Runtime is connected. Run workflow observe to choose plugin snapshot evidence or normal browser diagnosis. Use divebell --help when extension command discovery is needed."
  };
}
...
function createSnapshotObserveNextAction(pluginSnapshot, url) {
  const commands = [
    "divebell --help",
    "divebell snapshot --url <app-url>"
  ];
  return {
    type: "snapshot_observe",
    summary: "Plugin snapshot evidence is available. Inspect --help only when extension command discovery is useful, then run one full snapshot without --id/--query. If it is not useful, switch to Divebell browser diagnosis such as console/network/page-snapshot/eval.",
    plugins: pluginSnapshot.plugins,
    url: url ?? null,
    commands,
    rules: [
      "External extension commands are shown in --help under External Extensions.",
      "When the command section reports an available skill, run divebell <command> --skill and read the returned SKILL.md before using that command.",
...
function readOpenOperationLog(cwd) {
  const filePath = path.join(homedir(), ".divebell", "operations", `${createOperationLogKey(cwd)}.json`);
  try {
    const parsed = JSON.parse(fs.readFileSync(filePath, "utf8"));
