...
const baseUrl = ensureTrailingSlash(
  option("--base") ?? "https://2heal1.github.io/openruntime/quickstart/"
);
const outputDirectory = resolve(
  option("--output") ?? "/tmp/openruntime-quickstart-build"
);
const chunkMapUrl = new URL("openruntime-chunks.json", baseUrl);
...
const files = new Set(["openruntime-chunks.json"]);
...
await Promise.all([...files].map(async (file) => {
  ...
  const source = file === "openruntime-chunks.json"
    ? chunkMapSource
...
process.stdout.write(`${JSON.stringify({
  ...
  baseUrl,
  outputDirectory,
  chunkMap: join(outputDirectory, "openruntime-chunks.json"),
