...
const runtimeEnvironment = {
  ...process.env,
  DIVEBELL_EXTENSIONS_DIR: managedExtensionsDirectory
};
const userArgs = process.argv.slice(2);

const explicitCli = process.env.DIVEBELL_CLI;
...
try {
  ...
  runtimeFailure = [
    "Failed to install the Divebell CLI from the released runtime bundle.",
...
for (const command of ["divebell", "divebell"]) {
...
process.stderr.write([
  ...
  "Set DIVEBELL_CLI=/absolute/path/to/divebell, retry with network access, or provide DIVEBELL_SKILL_RUNTIME_ARCHIVE and its .sha256 file."
].filter(Boolean).join("\n") + "\n");
process.exit(127);
...
function supportsRecordCommand(invocation, env) {
  ...
  return result.status === 0 && result.stdout.includes("divebell record start [--out <path>]");
}
...
function installReleasedRecordCommand(manifest, packageDirectory) {
  const extensionPackage = manifest.packages.find((item) => item.name === "@divebell/extension-imitate");
  if (extensionPackage === undefined) {
    return {
      status: 1,
      stdout: "",
      stderr: "Recording runtime does not include @divebell/extension-imitate."
    };
  }
...
function installReleasedCli(manifest, packageDirectory, destination) {
  ...
  if (mkdirResult.status !== 0) {
    return {
      ...
      stdout: "",
      stderr: "Could not create the Divebell CLI install directory."
    };
  }
...
function getInstallTimeout() {
  const value = Number(process.env.DIVEBELL_SKILL_CLI_INSTALL_TIMEOUT_MS);
