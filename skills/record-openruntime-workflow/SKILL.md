...
description: 录制用户在已接入 OpenRuntime 的网页中的人工浏览器操作，并在用户说结束后由 Agent 通过 skill 自带的 OpenRuntime CLI wrapper 收尾、关闭浏览器、读取 .orrec 录制包并生成 JS 脚本草稿。Use when the user wants an agent-installable workflow skill for recording browser actions, converting recorded OpenRuntime evidence into an automation script, or drafting a reusable skill/script from a manual web workflow without assuming a global openruntime command.
---
...
- 第一版录制包会保存鼠标点击、输入、键盘事件、事件相对录制开始的时间、页面快照、DOM 摘要和 OpenRuntime 结构化状态。连续视频、真实麦克风音频和语音转文字还不是可靠产物；`--mic` 当前只记录用户请求，后续再接入真实音频和文字补充。
- 页面跳转、搜索或打开新页面后，中间的点击和输入也应该保留在 `interactions.jsonl`。不要只按最后停留的 URL 判断录制结果。
- 生成脚本后必须读取脚本、`manifest.json`、`interactions.jsonl` 和 `dom-snapshots.jsonl`，再向用户说明产物路径和当前限制。
...
- 没有可用候选时，告诉用户未发现可复用 CLI，将使用 skill 自带 CLI。不要因为没有全局命令而中断。
- 用户不想复用已检测到的 CLI 时，继续使用 skill 自带 CLI；用户也可以先自己安装指定版本，再重新探测。
...
结束后读取 `manifest.json`、`interactions.jsonl` 和 `generated-script.mjs`。确认 manifest 的 `status` 是 `completed`，并检查脚本中是否出现了录到的 `click`、`fill` 或键盘步骤。不要只因为页面没有 OpenRuntime target 就说“没有录到操作”；先看 `interactions.jsonl`。
...
- 鼠标和键盘信息来自 `interactions.jsonl`；页面上下文来自 `page-snapshots.jsonl` 和 `dom-snapshots.jsonl`；语音后续接入后按时间范围辅助解释这些事件。
