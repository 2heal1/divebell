...
本 skill 使用全局安装的 OpenRuntime CLI，不向当前业务项目添加依赖，也不临时下载另一
份 CLI。

## 安装

如果 `openruntime` 命令不存在，先全局安装：
...
`record stop` 生成的 `generated-script.mjs` 默认执行全局 `openruntime`。运行脚本前
用 `openruntime --help` 确认命令仍然可用。
