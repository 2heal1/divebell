...
const MANIFEST_FILE = "openruntime-cli-runtime.json";
...
export function resolveRuntimeLayout(manifest) {
  const cacheRoot = resolve(process.env.OPENRUNTIME_SKILL_CLI_HOME ??
...
export async function prepareRuntimePackages(manifest, layout) {
  ...
  try {
    const localArchive = process.env.OPENRUNTIME_SKILL_RUNTIME_ARCHIVE;
    if (localArchive !== undefined && localArchive.length > 0) {
      await copyFile(resolve(localArchive), archivePath);
    } else {
      await downloadFile(process.env.OPENRUNTIME_SKILL_RUNTIME_URL ?? manifest.asset.url, archivePath);
    }
...
async function resolveExpectedChecksum(manifest, localArchive) {
  const explicitChecksum = process.env.OPENRUNTIME_SKILL_RUNTIME_SHA256;
  ...
  if (localArchive !== undefined && localArchive.length > 0) {
    ...
    try {
      ...
      throw new Error("A local runtime archive requires OPENRUNTIME_SKILL_RUNTIME_SHA256 or a sibling .sha256 file.");
    }
  }

  const checksumUrl = process.env.OPENRUNTIME_SKILL_RUNTIME_SHA256_URL ?? manifest.asset.checksumUrl;
...
async function fetchWithTimeout(url) {
  ...
  const token = process.env.OPENRUNTIME_SKILL_RUNTIME_TOKEN;
  try {
...
function getDownloadTimeout() {
  const value = Number(process.env.OPENRUNTIME_SKILL_RUNTIME_DOWNLOAD_TIMEOUT_MS);
