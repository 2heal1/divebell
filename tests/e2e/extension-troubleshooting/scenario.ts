...
import { verifyTestCommand } from "@divebell/extension-troubleshooting/test";

import { createTroubleshootingRuntimeFixture } from "../support/runtime-fixture.js";
import type { DivebellE2eContext } from "../support/types.js";

export function registerTroubleshootingExtensionE2e({
  getEnvironment
  ...
  test("runs the installed troubleshooting Extension through the real CLI", async () => {
    ...
    try {
      const passed = await environment.runCli(
        verifyTestCommand(fixture.targetId, "ready", {
      ...
      const failed = await environment.runCli(
        verifyTestCommand(fixture.targetId, "error", {
          bridge: fixture.bridgeUrl,
          url: fixture.pageUrl,
          timeout: 30
        }),
